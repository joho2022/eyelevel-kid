...
class MyNotifier extends Notifier<MyState> {
  ...
  // MARK: - 유저 상태 구독
  void _observeUser() {
    _subscription = _observeUserUseCase().listen((user) {
  ...
  Future<void> refreshProfileImageUrl() async {
    ...
    try {
      await _refreshProfileImageUrlUseCase();
    } catch (error) {
  ...
  Future<void> changePreferredStyle(AnswerStyle style) async {
    try {
      await _updateAnswerStyleUseCase(style);
    } catch (error) {
  ...
  String? _getReviewUrl() {
    if (Platform.isIOS) return _iosReviewUrl;
    if (Platform.isAndroid) return _androidMarketUrl;
    return null;
  }

  String? _getShareStoreUrl() {
    if (Platform.isIOS) return _iosAppStoreUrl;
    if (Platform.isAndroid) return _androidPlayStoreWebUrl;
    return null;
  }
